...
export const VideoIdField = forwardRef((props, ref) => {
  const {
  ...
  const [newDoc, setNewDoc] = useState<VideoType>(null);
  ...
  const handleChange = useCallback(
    async (e) => {
      ...
      let inputValue: VideoType = null;

      if (url.trim().length) {
        const { id, service } = getVideoId(url);
        const thumbnail = await getThumbnail({ id, service });
        inputValue = { url, id, service, thumbnail };
      }

      setNewDoc(inputValue);
      onChange(PatchEvent.from(url === "" ? set(inputValue) : unset()));
    },
    [onChange]
  );
